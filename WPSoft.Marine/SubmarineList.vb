...
Friend NotInheritable Class SubmarineList
    Friend Shared ReadOnly List As New List(Of Canvas)

    Friend Shared Sub Send()
        Dim target = SpriteAPI.[New](New Random().Next)
        target.Background = New ImageBrush(New BitmapImage(PathAPI.GetUri(WADV.Core.PathType.Resource, "submarine.png")))
        target.Margin = New Thickness(-50, New Random().Next(60, 100), 0, 0)
