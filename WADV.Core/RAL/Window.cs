...
namespace WADV.Core.RAL {
    public abstract class Window : IDisposable {
        ...
        public bool FullScreen {
            ...
            set {
                if (!CanFullscreen) {
                    new GameException().Type(ExceptionType.FullScreenNotAllowed)
                        .ExceptionValue(value.ToString())
                        .Source("RAL.Window")
                        .Message("Full screen is not allowed by window")
                        .Save()
                        .AsThrowable();
                    return;
                }
        ...
        public void Close() {
            if (!CanClose) {
                new GameException().Type(ExceptionType.FullScreenNotAllowed)
                    .ExceptionValue(GameException.NoValue)
                    .Source("RAL.Window")
                    .Message("Close is not allowed by window")
                    .Save()
                    .AsThrowable();
                return;
            }
        ...
        protected abstract bool Closing();
